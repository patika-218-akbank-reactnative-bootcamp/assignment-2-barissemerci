...
const ChatView = ({conversation}) => {
  ...
  const renderChats = ({item}) => {
    if (item.messages.image) {
      return (
        <View
          style={item.isMyMessage === 1 ? styles.myImageBox : styles.imageBox}>
          ...
          <View>
            <Text
              ...
              {Moment(item.messages.datetime).format('HH:mm')}
            </Text>
            {item.isMyMessage === 1 && (
              <IconCheck
      ...
      return (
        <View
          style={
            item.isMyMessage === 1 ? styles.myMessageBox : styles.messageBox
          }>
